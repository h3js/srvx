...
- `dirListing`: Serve a minimal HTML directory listing when a request resolves to a directory with no index file (default `false`). Off by default because it exposes the directory structure — it is opt-in. The [CLI](/guide/cli#serving-static-files) turns it on in dev mode only.
