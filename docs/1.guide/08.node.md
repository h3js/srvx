...
To avoid these issues, you can use `req.clone()` or `req._request` to access an undici instance of request. Alongside it, `req._url` exposes the already-parsed `URL` for the request.
...
`FastResponse` keeps the body you passed in exactly as you passed it — a string stays a string all the way to `res.write()`, and its `Content-Type` and `Content-Length` are derived from it at send time. Reading `response.body` gives up that fast path: it forces the full [Response][Response] internals to be built and turns the body into a [ReadableStream][ReadableStream].

So to add a header — a `Set-Cookie`, a CORS or cache header, anything a middleware wants to attach to a response it did not create — **mutate `response.headers` in place** rather than rebuilding the response around its body:

```ts
// ✅ Fast: the body is never touched
res.headers.append("set-cookie", "session=abc; HttpOnly");
res.headers.set("cache-control", "no-store");
return res;

// ❌ Slow: reading `res.body` materializes the response
return new FastResponse(res.body, {
  status: res.status,
  headers: { ...Object.fromEntries(res.headers), "set-cookie": "session=abc; HttpOnly" },
});
```

Rebuilding costs roughly **2x** the throughput of mutating on a small response, because the raw body is replaced by a stream that has to be pumped chunk by chunk instead of written once with a known length.

`response.headers` on a `FastResponse` is mutable, whether or not the body has already been materialized, and mutations are guaranteed to reach the wire. This also holds for a plain `new Response()` on Node.js, Deno and Bun, so a middleware can mutate without knowing which class it was handed. The exceptions are responses whose headers the [Fetch standard](https://fetch.spec.whatwg.org/#concept-headers-guard) marks immutable — those returned by `fetch()`, `Response.error()` and `Response.redirect()`. For those, rebuilding is the only option, and it costs nothing: their bodies are already streams.

> [!NOTE]
> A `FastResponse` may only be sent once. Its body and headers are released as it is written to the socket, so a cached instance replayed across requests sends an empty body the second time. Build a new response per request, or cache the body and construct around it.
