...
> - **Bun**: the native `Bun.serve` enforces `requestCert`/`rejectUnauthorized` at the handshake but does not expose the peer certificate to the handler, so `request.tls` is unavailable on that path. Import srvx's Node adapter instead — `import { serve } from "srvx/node"` — which runs on Bun via `node:https` and exposes `request.tls` (verified on Bun 1.3.14; earlier versions did not, see [oven-sh/bun#16254](https://github.com/oven-sh/bun/issues/16254)).
