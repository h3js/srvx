...
- `ca`: Path(s) or inline CA certificate(s) in PEM format used to verify client certificates for mutual TLS (optional).
- `requestCert`: Request a certificate from connecting clients, enabling mutual TLS (default `false`). The presented certificate is available via [`request.tls`](/guide/handler#requesttls).
- `rejectUnauthorized`: Reject clients whose certificate is not signed by a trusted `ca` (default `true` when `requestCert` is enabled).
...
> [!NOTE]
> mTLS is supported natively on Node.js. On Deno (2.8+) and Bun, use srvx's Node adapter (`import { serve } from "srvx/node"`), which runs on those runtimes via `node:https` and exposes `request.tls`; the native Deno server throws if `requestCert` is set, and native `Bun.serve` enforces the handshake but does not expose the client certificate to the handler. See [`request.tls`](/guide/handler#requesttls).

> [!TIP]
> You can pass inline `cert`, `key` and `ca` values in PEM format starting with `-----BEGIN `.
