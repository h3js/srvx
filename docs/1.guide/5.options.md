...
Mutual TLS — requesting and verifying a client certificate — is provided by the opt-in `mtls()` plugin from `srvx/mtls`. It requests the certificate during the handshake and exposes it on [`request.tls`](/guide/handler#requesttls).
...
serve({
  ...
  fetch: (request) => new Response(`Hello, ${request.tls?.peerCertificate?.subject?.CN}`),
});
```

**`mtls()` options:**

- `ca`: Path(s) or inline CA certificate(s) in PEM format used to verify client certificates (optional). When set, the well-known Mozilla CAs are replaced by the provided ones.
- `requestCert`: Request a certificate from connecting clients (default `true`).
- `rejectUnauthorized`: Reject clients whose certificate is not signed by a trusted `ca` (default `true`). When `false`, an unverified certificate is still exposed via `request.tls` with `authorized: false`.

> [!NOTE]
> `mtls()` requires srvx's **Node.js adapter** (`import { serve } from "srvx/node"`) and throws on any other server. It works on Node.js and on Deno (2.8+, via `node:https`). It is not currently supported on Bun, which does not expose the peer certificate to `node:http(s)` handlers ([oven-sh/bun#16254](https://github.com/oven-sh/bun/issues/16254)). See [`request.tls`](/guide/handler#requesttls).
