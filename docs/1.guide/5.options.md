...
> [!NOTE]
> mTLS is supported natively on Node.js. On Deno (2.8+), use srvx's Node adapter (`import { serve } from "srvx/node"`), which runs on Deno via `node:https`; the native Deno server throws if `requestCert` is set. Bun enforces the handshake but does not expose the client certificate to the handler. See [`request.tls`](/guide/handler#requesttls).
