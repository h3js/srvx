...
A middleware that decorates a response — a `Set-Cookie`, a CORS or cache header — should mutate `res.headers` in place, as `xPoweredBy` does above. Rebuilding the response around its body (`new Response(res.body, { ... })`) reads `res.body`, which on Node.js discards the [`FastResponse`](/guide/node#fastresponse) fast path and costs roughly 2x the throughput on a small response.

Response headers are mutable on every runtime srvx supports, so this works regardless of which class the handler returned. The exception is a response from `fetch()`, `Response.error()` or `Response.redirect()`, whose headers the Fetch standard marks immutable — rebuild those instead.
