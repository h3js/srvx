...
If a request nevertheless reaches the plugin over a plain (non-TLS) socket, it is answered with `496 Client certificate required` instead of running your handler with `request.tls` left `undefined`.

### Running a TLS entry through the CLI

An entry that configures `tls` (and `mtlsPlugin()`) keeps both when it is started with the [`srvx` CLI](/guide/cli): the CLI serves it over `https://` and preserves the client-certificate requirement. `--tls --cert --key` still take precedence over the entry's own settings. See [TLS precedence](/guide/cli#tls-precedence).
