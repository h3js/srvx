...
The CLI serves over HTTPS whenever TLS is configured, resolved with the following precedence (highest first):

1. CLI flags (`--tls --cert --key`) &mdash; `--tls` requires both `--cert` and `--key`
2. Module option (`tls` exported from your server entry)
3. The `tls` passed to a `serve()` call the entry makes itself (see [Loading entries that start a server](#loading-entries-that-start-a-server))

An entry configured for TLS is therefore served over `https://` under the CLI too, and the listening URL the CLI prints reflects that. As with a direct run, TLS also enables HTTP/2 (HTTP/1.1 still allowed). Mutual TLS configured through [`mtlsPlugin()`](/guide/tls#mutual-tls-mtls) is carried over as well, so the client-certificate requirement stays enforced.
...
Anything the entry exports alongside its handler is on `loaded.module`, so you can read its [server options](/guide/options) and merge them with your own. Options the entry passed to its own `serve()` call are on `loaded.srvxServer.options` instead &mdash; forward the listener-level ones (`tls`, `protocol`, `node`) to your own `serve()`, as the srvx CLI does, or an entry configured for TLS ends up served in cleartext. Do **not** forward `middleware`/`plugins`: `loaded.fetch` already has them applied, so re-adding them would run them twice.
