...
On the Node adapter the [`req._request`](/guide/node#noderequest) escape hatch is served over the same size-limited stream, so unwrapping the request — `req._request`, or `new Request(req)` after `patchGlobalRequest()` — cannot reach the raw body and bypass the limit.
...
Both helpers reject with the canonical error created by `createBodyTooLargeError` (typed as the exported `BodyTooLargeError`). It carries a stable, documented shape so any layer can map it to an HTTP `413 Content Too Large` response without string matching:
