...
Both helpers reject with the canonical error created by `createBodyTooLargeError`. It carries a stable, documented shape so any layer can map it to an HTTP `413 Payload Too Large` response without string matching:
